###############################################################################
# report_A_ooc.R  (out-of-core version of report_A.R)
#
# Same outputs as report_A (instance turmas/escolas, descriptive/eval/fairness/
# interpretability tables+figures, Leia-me), but it consumes the PARTITIONED
# outputs of the out-of-core pipeline (features_{y}_p{k}.rds, risk_{y}_p{k}.rds)
# instead of one file per year. All per-school / per-turma / per-stratum
# aggregations are done as a REDUCE over partitions (aggregate within a partition,
# then sum across partitions), so nothing scales with the full-year row count.
#
# Output: <OUT>/{instance,tables,figures}/ + Leia-me.(txt|pdf)
###############################################################################

suppressPackageStartupMessages({ library(data.table); library(ggplot2); library(mgcv) })
source(file.path("scripts", "ooc_common.R"))

# ─── CONFIG (env-overridable; point at the pipeline's T: outputs in the room) ─
RISK_DIR     <- Sys.getenv("REPORT_RISK",  "data/model_ooc")
FEATURES_DIR <- Sys.getenv("REPORT_FEAT",  "data/features_ooc")
CLEAN_DIR    <- Sys.getenv("REPORT_CLEAN", "data/clean_ooc")
OUT          <- Sys.getenv("REPORT_OUT",   "outputs/report_A")
YEAR         <- as.integer(Sys.getenv("REPORT_YEAR", "2024"))     # instance / scoring year
TEST_YEAR    <- as.integer(Sys.getenv("REPORT_TEST", "2023"))     # last labeled year, for eval
LABEL_YEARS  <- eval(parse(text = Sys.getenv("REPORT_LABELS", "2019:2023")))
MIN_CELL     <- 10L
MODELS       <- c("logit", "gam", "lgb")
EMIT_MODEL_DEPENDENT <- TRUE

SCOPE_NAME <- Sys.getenv("REPORT_SCOPE", "nacional")
SCOPES <- list(
...
# union of a per-year partitioned file (risk/features), optionally column-subset
read_year_parts <- function(dir, stem, y, cols = NULL) {
  fs <- Filter(file.exists, lapply(0:(K_PARTS - 1L), function(k) part_path(dir, stem, y, k)))
  if (!length(fs)) return(NULL)
  rbindlist(lapply(fs, function(f) { d <- readRDS(f); if (is.null(cols)) d else d[, ..cols] }),
            fill = TRUE)
}
...
suppress <- function(dt) dt[n >= MIN_CELL]
guard <- function(label, expr) tryCatch(expr,
  error = function(e) message("  [SKIP ", label, "] ", conditionMessage(e)))

# ─── METRICS (auc_roc in doubles: avoid integer overflow at national scale) ───
...
# ═══ BLOCK 1 — INSTANCE (reduce risk mass by school/turma across partitions) ══
guard("instancia", {
  esc_acc <- NULL; tur_acc <- NULL
  for (k in 0:(K_PARTS - 1L)) {
    rk <- part_path(RISK_DIR, "risk", YEAR, k); fk <- part_path(FEATURES_DIR, "features", YEAR, k)
    if (!file.exists(rk) || !file.exists(fk)) next
    s <- merge(readRDS(rk)[, .(CO_PESSOA_FISICA, r_logit, r_gam, r_lgb)],
               readRDS(fk)[, .(CO_PESSOA_FISICA, ID_TURMA, CO_ENTIDADE)], by = "CO_PESSOA_FISICA")
    esc_acc <- rbind(esc_acc, s[, .(n = .N, R_sigma_logit = sum(r_logit), R_sigma_gam = sum(r_gam),
                                    R_sigma_lgb = sum(r_lgb)), by = CO_ENTIDADE])
    tur_acc <- rbind(tur_acc, s[, .(n = .N, CO_ENTIDADE = CO_ENTIDADE[1], R_t_logit = sum(r_logit),
                                    R_t_gam = sum(r_gam), R_t_lgb = sum(r_lgb)), by = ID_TURMA])
    rm(s)
  }
  if (is.null(esc_acc)) stop("no risk/features partitions for year ", YEAR)
  # a school/turma appears in several partitions (students spread) -> sum partials
  esc_agg <- esc_acc[, .(n = sum(n), R_sigma_logit = sum(R_sigma_logit), R_sigma_gam = sum(R_sigma_gam),
                         R_sigma_lgb = sum(R_sigma_lgb)), by = CO_ENTIDADE]
  tur_agg <- tur_acc[, .(n = sum(n), CO_ENTIDADE = CO_ENTIDADE[1], R_t_logit = sum(R_t_logit),
                         R_t_gam = sum(R_t_gam), R_t_lgb = sum(R_t_lgb)), by = ID_TURMA]
  ...
  turmas <- merge(tur_agg, tur_raw[, .(ID_TURMA, TP_ETAPA_ENSINO)], by = "ID_TURMA", all.x = TRUE)
...
# ═══ BLOCK 2 — DESCRIPTIVE (streamed year x partition, no pooling) ════════════
guard("descritivas", {
  ...
  for (y in LABEL_YEARS) {
    ...
    ereg <- if (!is.null(e)) e[, .(CO_ENTIDADE, regiao = REGIAO[substr(CO_UF, 1, 1)])] else NULL
    for (k in 0:(K_PARTS - 1L)) {
      fp <- part_path(FEATURES_DIR, "features", y, k); if (!file.exists(fp)) next
      f <- readRDS(fp)
      if (!is.null(ereg)) f <- merge(f, ereg, by = "CO_ENTIDADE", all.x = TRUE)
      f <- f[!is.na(evadiu)]
...
if (EMIT_MODEL_DEPENDENT) {
  risk_te <- read_year_parts(RISK_DIR, "risk", TEST_YEAR,
                             cols = c("CO_PESSOA_FISICA","evadiu","r_logit","r_gam","r_lgb"))
  risk_te <- risk_te[!is.na(evadiu)]
  yte <- risk_te$evadiu
  ...
  guard("fairness", {
    fte <- read_year_parts(FEATURES_DIR, "features", TEST_YEAR,
                           cols = c("CO_PESSOA_FISICA","CO_ENTIDADE","TP_DEPENDENCIA","TP_COR_RACA"))
...
} else message("EMIT_MODEL_DEPENDENT=FALSE — blocos 3-5 pulados.")
